...
// ══════════════════════════════════════════════════════
// TIH Stocks v6.0
// مبني على: مورفي + القاسم + وايكوف + أبو سهيل
// أوبشن أسابيع — أهداف مبنية على Fib + EMA + BB
// ══════════════════════════════════════════════════════
...
const MIN_SCORE      = 12;
const MIN_SIGNAL_GAP = 8 * 60 * 60 * 1000; // 8 ساعات للأسهم
...
async function getBars(sym, interval, range) {
  const yahooSym = sym === '^VIX' ? '^VIX' : sym;
...
function detectMarketStructure(highs, lows, closes) {
  ...
  if (swingHighs.length < 2 || swingLows.length < 2) {
    ...
    const fh = Math.max(...recent_h.slice(0,mid)), sh = Math.max(...recent_h.slice(mid));
    const fl = Math.min(...recent_l.slice(0,mid)), sl = Math.min(...recent_l.slice(mid));
    if (sh > fh && sl > fl) return 'bull';
    if (sh < fh && sl < fl) return 'bear';
    return 'neutral';
  }
  ...
  if (lastH > prevH && lastL > prevL) return 'bull';
  if (lastH < prevH && lastL < prevL) return 'bear';
  return 'neutral';
}

// ══════════════════════════════════════
// Stop Hunt — وايكوف (Spring/Upthrust)
// ══════════════════════════════════════
...
function detectCandlePattern(highs, lows, closes, opens, signal) {
  ...
  if (signal === 'CALL') {
    ...
    // Bullish Engulfing — القاسم
    if (c1<o1 && c>o && c>=o1 && o<=c1 && range>range1) return '🟢 Engulfing↑';
    // Marubozu صاعد — جسد ضخم بدون ظلال
    if (isBull && tailDn<=0.05 && tailUp<=0.05 && body>=0.85) return '🔥 Marubozu↑';
    // Dragonfly Doji
    if (tailDn>=0.70 && body<=0.10) return '🐉 Dragonfly Doji';
    // Piercing Pattern
    if (c1<o1 && c>o && o<l1 && c>(o1+c1)/2 && c<o1) return '🔰 Piercing';
    // Morning Star — 3 شموع: هبوط + دوجي/صغيرة + صعود
    if (c2<o2 && Math.abs(c1-o1)<(h1-l1)*0.3 && c>o && c>(o2+c2)/2) return '🌅 Morning Star';
    // Three White Soldiers — القاسم
    if (len>=3 && c>o && c1>o1 && closes[i-2]>opens[i-2] &&
        c>c1 && c1>closes[i-2] && o>o1 && o1>opens[i-2]) return '⬆️ Three White Soldiers';
    // Inverted Hammer بعد هبوط
    if (tailUp>=0.55 && body<=0.35 && tailDn<=0.10) return '🔨 Inverted Hammer';
  } else {
    // Shooting Star
    ...
    // Gravestone Doji
    if (tailUp>=0.70 && body<=0.10) return '🪦 Gravestone Doji';
    // Dark Cloud Cover
    if (c1>o1 && c<o && o>h1 && c<(o1+c1)/2 && c>o1) return '☁️ Dark Cloud';
    // Evening Star — القاسم
    if (c2>o2 && Math.abs(c1-o1)<(h1-l1)*0.3 && c<o && c<(o2+c2)/2) return '🌆 Evening Star';
    // Three Black Crows — القاسم
    if (len>=3 && c<o && c1<o1 && closes[i-2]<opens[i-2] &&
        c<c1 && c1<closes[i-2] && o<o1 && o1<opens[i-2]) return '⬇️ Three Black Crows';
    // Hanging Man بعد صعود
    if (tailDn>=0.55 && body<=0.35 && tailUp<=0.10) return '🙇 Hanging Man';
  }
  // Harami — القاسم: شمعة صغيرة داخل السابقة
...
function hasVolumeConfirmation(bars) {
  ...
  return vols[vols.length-1] >= avgVol * 1.1; // للأسهم: 1.1x
}

// ══════════════════════════════════════
// Weis Volume Analysis — وايكوف
// Absorption: حجم ضخم مع حركة سعر ضعيفة
// ══════════════════════════════════════
function detectAbsorption(highs, lows, closes, vols, signal) {
  ...
  const avgRange = closes.slice(-10).map((_,i,arr) => i>0?Math.abs(arr[i]-arr[i-1]):0)
                         .reduce((a,b)=>a+b,0)/9;
  // حجم كبير لكن حركة سعر صغيرة = Absorption = يد قوية تمتص
...
// ══════════════════════════════════════
// حساب الأهداف — للأسهم (أسابيع)
// يعتمد على: Fib + EMA + BB + PDH/PDL
// ══════════════════════════════════════
function calcTargetsStocks(signal, price, atrVal, levels) {
  ...
  for (const c of t1C) { if(Math.abs(c.val-price)/risk>=2.0){t1=+c.val.toFixed(2);t1Label=c.label;break;} }

  let t2=+(price+d*risk*3.5).toFixed(2), t2Label='1:3.5R';
  for (const c of t2C) { if(Math.abs(c.val-price)/risk>=3.0&&Math.abs(c.val-price)>Math.abs(t1-price)){t2=+c.val.toFixed(2);t2Label=c.label;break;} }

  let t3=+(price+d*risk*6).toFixed(2), t3Label='1:6R';
  for (const c of t3C) { if(Math.abs(c.val-price)/risk>=5.0&&Math.abs(c.val-price)>Math.abs(t2-price)){t3=+c.val.toFixed(2);t3Label=c.label;break;} }
  ...
  if (t1Pct < 2.0) thetaWarning = `⚠️ Theta: T1 قريب (${t1Pct.toFixed(1)}%) — Delta ≥ 0.55`;
  else if (t1Pct < 3.5) thetaWarning = `⚡ Theta: استخدم Delta 0.45-0.55`;
...
async function analyzeMTF(sym, vix) {
  ...
  // Fibonacci Extensions من اليومي
  const fib = entryBars
    ? calcFibExtensions(dailyBars.closes, dailyBars.highs, dailyBars.lows, 'CALL') // يُحدَّث لاحقاً
    : null;
  ...
  // Weekly Trend يجب أن يتوافق — إلزامي للأسهم
  ...
  // Weis Absorption — وايكوف
  const absorption = entryBars && entryBars.vols
    ? detectAbsorption(entryBars.highs, entryBars.lows, entryBars.closes, entryBars.vols, requiredSignal)
    : false;
  ...
  // Candle Pattern — القاسم (على اليومي للأسهم)
  const candlePatternDaily = dailyBars
    ? detectCandlePattern(dailyBars.highs, dailyBars.lows, dailyBars.closes, dailyBars.opens, requiredSignal)
    : null;
  const candlePatternEntry = entryBars
    ? detectCandlePattern(entryBars.highs, entryBars.lows, entryBars.closes, entryBars.opens, requiredSignal)
    : null;
  const candlePattern = candlePatternDaily || candlePatternEntry;
  ...
  // Fib Confluence — هل السعر قريب من مستوى Fib مهم؟
  let fibConfluence = null;
  if (fibLevels) {
    const price = entryData.price || dailyBars.price;
    for (const [key, val] of Object.entries(fibLevels)) {
      if (val && Math.abs(price-val)/val*100 <= 1.0) {
        fibConfluence = `Fib ${key.replace('fib','')}`;
        break;
      }
    }
  }
  ...
  const bonuses = (ictScore>=5?2:ictScore>=3?1:0)
                + (stopHunt.detected?3:0)
                + (candlePattern?2:0)
                + (absorption?2:0)        // وايكوف Absorption
                + (fibConfluence?2:0)      // Fib Confluence — مورفي
                + (volOk?1:0);
...
module.exports = async (req, res) => {
  ...
  if(action==='test'){
    ...
    await tg(
      ...
      `✅ Candle Patterns (9 أنماط)\n`+
      `✅ Weis Absorption\n`+
      `✅ Fibonacci Extensions\n`+
      `✅ PDH/PDL\n`+
      `✅ أهداف أسابيع مع Theta Warning\n`+
  ...
  if(vix&&vix>25){
    ...
    if(lastVixAlert!==today){
      ...
      await tg(vix>35
        ?`⚠️ <b>VIX شديد!</b> ${vix.toFixed(1)} — تداول بحذر\n🤖 TIH Stocks v6.0`
        :`⚠️ <b>VIX مرتفع</b> ${vix.toFixed(1)} — راقب الإشارات\n🤖 TIH Stocks v6.0`);
    }
  }
  ...
  await Promise.all(symbols.map(async (sym)=>{
    try{
      ...
      const targets=calcTargetsStocks(result.signal,result.price,result.atr,result.levels);
      ...
      const absorptionLine=result.absorption
        ?`📦 Weis Absorption — يد قوية تمتص\n`:'';
      const fibLine=result.fibConfluence
        ?`📐 Fib Confluence: ${result.fibConfluence}\n`:'';
      ...
      await tg(
        ...
        `${weekLine}${stopHuntLine}${candleLine}${absorptionLine}${fibLine}${ictLine}`+
